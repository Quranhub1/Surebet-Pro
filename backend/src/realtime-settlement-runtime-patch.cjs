...
if (realtimeSettlementService && typeof realtimeSettlementService.getEventsForDate === 'function') {
  ...
  realtimeSettlementService.findBestMatch = function(events, row) {
    ...
    for (const event of unique) {
      ...
      if (teamScore < 1.35) continue;
      const eventKickoff = event?.event_date ?? event?.kickoff_at ?? event?.kickoff ?? event?.date ?? event?.start_time ?? event?.startTime ?? event?.event?.event_date ?? event?.event?.kickoff_at ?? event?.event?.date;
      const kickoffScore = eventKickoff && this.kickoffMatches(eventKickoff, row.kickoff_at) ? 0.25 : eventKickoff ? -0.5 : 0;
  ...
  console.log('[History] BSD settlement runtime patch loaded: shared date-feed matching, event_date support, robust team extraction, cache stampede prevention, weak-match blocking, and slow per-team searches disabled.');
}
