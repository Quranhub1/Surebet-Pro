...
export class ScannerScheduler {
  ...
  public async start(): Promise<void> {
    ...
    console.log('[AI] Upcoming games are stored in Neon and historical completed games are fed back into future analysis.');
  ...
  private scheduleNextAnalysis(): void {
    ...
    console.log(`[AI] Next automatic football analysis in approximately ${(ANALYSIS_INTERVAL_MS / 3600000).toFixed(0)} hours.`);
  }
  ...
  private async runAnalysisIfDue(): Promise<void> {
    try {
      ...
      else console.log(`[AI] Previous analysis is still current. Next cycle is due in approximately ${((ANALYSIS_INTERVAL_MS - (Date.now() - lastRunAt)) / 3600000).toFixed(1)} hours.`);
    } catch (error) {
  ...
  private async executeAnalysis(): Promise<void> {
    try {
      ...
      await sql`UPDATE system_settings SET last_run_at = NOW(), last_run_status = 'error' WHERE id = 1`;
    }
  }
}
