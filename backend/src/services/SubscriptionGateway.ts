...
async function isAdmin(userId: string): Promise<boolean> {
  ...
  return String(rows[0]?.email || '').trim().toLowerCase() === adminEmail;
}
...
async function subscriptionState(userId: string) {
  ...
  if (!rows[0].trial_started_at) {
    rows = await sql`UPDATE users SET trial_started_at = NOW() WHERE id = ${userId} AND trial_started_at IS NULL RETURNING id, email, trial_started_at, subscription_status, subscription_expires_at, subscription_requested_at`;
  }
  const row = rows[0];
  ...
  if (String(row.subscription_status) === 'active' && subscriptionExpiresAt && subscriptionExpiresAt.getTime() <= Date.now()) {
    await sql`UPDATE users SET subscription_status = 'expired' WHERE id = ${userId} AND subscription_status = 'active'`;
  }
  return {
    trialActive,
    trialStartedAt: trialStartedAt.toISOString(),
    trialExpiresAt: trialExpiresAt.toISOString(),
    subscriptionStatus: active ? 'active' : String(row.subscription_status) === 'pending' ? 'pending' : String(row.subscription_status) === 'active' ? 'expired' : String(row.subscription_status),
    subscriptionExpiresAt: subscriptionExpiresAt?.toISOString() || null,
    paymentAmountUgx: PAYMENT_AMOUNT_UGX,
...
async function requireAppSubscription(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  try {
    ...
    return res.status(503).json({ ok: false, error: 'Subscription service temporarily unavailable' });
  }
}
...
function registerRoutes(app: express.Application): void {
  app.get('/api/subscription', async (req, res) => {
    ...
    try {
      ...
      const state = await subscriptionState(userId);
      res.json({ ok: true, isAdmin: admin, subscription: state });
    } catch (error) {
      console.error('[Subscription] State lookup failed:', error);
      res.status(503).json({ ok: false, error: 'Failed to load subscription status' });
    }
  });
  ...
  app.post('/api/subscription/request', async (req, res) => {
    ...
    try {
      ...
      const state = await subscriptionState(userId);
      res.json({ ok: true, subscription: state, message: 'Payment marked as submitted. An administrator must verify the payment before the 7-day subscription is activated.' });
    } catch (error) {
      console.error('[Subscription] Payment request failed:', error);
      res.status(500).json({ ok: false, error: 'Could not submit payment request' });
    }
  });

  app.get('/api/admin/subscriptions/users', async (req, res) => {
  ...
  app.post('/api/admin/subscriptions/:userId/approve', async (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, user: { id: String(rows[0].id), email: String(rows[0].email), subscriptionStatus: String(rows[0].subscription_status), subscriptionExpiresAt: new Date(rows[0].subscription_expires_at).toISOString() } });
    } catch (error) {
      console.error('[Admin] Subscription approval failed:', error);
      res.status(500).json({ ok: false, error: 'Failed to approve subscription' });
    }
  });

  app.post('/api/admin/subscriptions/:userId/revoke', async (req, res) => {
    if (!await requireAdmin(req, res)) return;
    try {
      ...
      if (!rows[0]) return res.status(404).json({ ok: false, error: 'User not found' });
      res.json({ ok: true, user: { id: String(rows[0].id), email: String(rows[0].email), subscriptionStatus: String(rows[0].subscription_status) } });
    } catch (error) {
      console.error('[Admin] Subscription revoke failed:', error);
      res.status(500).json({ ok: false, error: 'Failed to revoke subscription' });
    }
  });
}
...
export function installSubscriptionGateway(): void {
  ...
  for (const method of ['get', 'post', 'patch', 'delete', 'put']) {
    ...
    proto[method] = function patchedRoute(path: any, ...handlers: any[]) {
      if (typeof path === 'string' && path.startsWith('/api/') && !PUBLIC_PATHS.has(path) && !path.startsWith('/api/admin/')) {
        return original.call(this, path, requireAppSubscription, ...handlers);
      }
