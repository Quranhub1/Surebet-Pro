...
export interface SubscriptionInfo { id: string; userId: string; plan: SubscriptionPlan; status: SubscriptionStatus; trialEndsAt: string; requestedAt: string | null; approvedAt: string | null; expiresAt: string | null; }
...
export async function getEffectiveSubscription(userId: string): Promise<SubscriptionInfo> {
  await ensureSubscriptionSchema();
  if (await isAdmin(userId)) return ensureUserSubscription(userId);
...
export async function requestPayment(userId: string, requestedPlan: string = DEFAULT_PLAN): Promise<SubscriptionInfo> {
  if (await isAdmin(userId)) throw new Error('Admin accounts are permanently active and do not need a subscription request.');
...
export async function listSubscriptions(): Promise<any[]> {
  ...
  return sql`SELECT s.id, s.user_id, u.name, u.email, u.role, s.plan,
    CASE WHEN UPPER(COALESCE(u.role, '')) = 'ADMIN' THEN 'permanent_active' ELSE s.status END AS status,
    s.trial_ends_at, s.requested_at,
    CASE WHEN UPPER(COALESCE(u.role, '')) = 'ADMIN' THEN COALESCE(s.approved_at, s.created_at) ELSE s.approved_at END AS approved_at,
    CASE WHEN UPPER(COALESCE(u.role, '')) = 'ADMIN' THEN NULL ELSE s.expires_at END AS expires_at,
    s.created_at, s.updated_at
    FROM subscriptions s JOIN users u ON u.id = s.user_id
    ORDER BY CASE WHEN s.status = 'pending' AND UPPER(COALESCE(u.role, '')) <> 'ADMIN' THEN 0 WHEN s.status = 'expired' THEN 1 WHEN s.status = 'active' THEN 2 WHEN s.status = 'trial' THEN 3 WHEN s.status = 'banned' THEN 4 WHEN UPPER(COALESCE(u.role, '')) = 'ADMIN' THEN 5 ELSE 6 END, s.updated_at DESC`;
}

async function getSubscriptionTarget(subscriptionId: string): Promise<{ userId: string; role: string; plan: SubscriptionPlan }> {
  const rows = await sql`SELECT s.user_id, s.plan, u.role FROM subscriptions s JOIN users u ON u.id = s.user_id WHERE s.id = ${subscriptionId} LIMIT 1`;
  ...
  return { userId: String(rows[0].user_id), role: String(rows[0].role || '').toUpperCase(), plan };
}

export async function approveSubscription(subscriptionId: string): Promise<SubscriptionInfo> {
  await ensureSubscriptionSchema();
  const target = await getSubscriptionTarget(subscriptionId);
  if (target.role === ADMIN_ROLE) return getEffectiveSubscription(target.userId);
  const durationDays = PAYMENT_PLANS[target.plan].durationDays;
  await sql`UPDATE subscriptions SET status = 'active', approved_at = NOW(), requested_at = COALESCE(requested_at, NOW()), expires_at = NOW() + (${durationDays} || ' days')::interval, updated_at = NOW() WHERE id = ${subscriptionId}`;
