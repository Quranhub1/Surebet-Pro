...
export class AiPredictionService {
    ...
    for (let batchStart = 0; batchStart < enriched.length; batchStart += AI_BATCH_SIZE) {
      ...
      try {
        ...
        for (const item of aiItems) {
          ...
          if (!match) { const home = item?.homeTeam ?? item?.home_team ?? item?.home; const away = item?.awayTeam ?? item?.away_team ?? item?.away; if (home && away) match = byTeams.get(`${String(home).trim().toLowerCase()}|${String(away).trim().toLowerCase()}`); }
          if (!match || generatedIds.has(String(match.id))) continue;
          generatedIds.add(String(match.id));
          const prediction: AiMatchPrediction = { id: String(match.id), league: match.league, homeTeam: match.home, awayTeam: match.away, startTime: match.kickoff, winner: item?.winner === match.home || item?.winner === match.away ? item.winner : item?.winner === 'Draw' ? 'Draw' : null, advice: this.stringOrNull(item?.advice), analysis: this.stringOrNull(item?.analysis), keyFactors: Array.isArray(item?.keyFactors) ? item.keyFactors.filter((v: unknown): v is string => typeof v === 'string').slice(0, 6) : [], confidence: this.toNumber(item?.confidence), homeWin: this.toNumber(item?.homeWin), draw: this.toNumber(item?.draw), awayWin: this.toNumber(item?.awayWin), underOver: this.stringOrNull(item?.underOver), predictedHomeGoals: this.toNumber(item?.predictedHomeGoals), predictedAwayGoals: this.toNumber(item?.predictedAwayGoals), aiProvider: usedProvider, aiModel: usedProvider === 'gemini' ? (process.env.GEMINI_MODEL || 'gemini-3.5-flash-lite') : (process.env.GROQ_MODEL || 'openai/gpt-oss-120b') };
  ...
  private async getProviderPreference(configured: AiProvider[], preferred: AiProvider): Promise<AiProvider[]> {
    ...
    try {
      ...
      for (const row of rows as any[]) { const evaluated = Number(row.evaluated) || 0; const correct = Number(row.correct) || 0; scores.set(String(row.ai_provider), evaluated >= 5 ? correct / evaluated : 0.5); }
      return [...configured].sort((a, b) => { const scoreDiff = (scores.get(b) ?? 0.5) - (scores.get(a) ?? 0.5); if (Math.abs(scoreDiff) > 0.01) return scoreDiff; if (a === preferred) return -1; if (b === preferred) return 1; return a.localeCompare(b); });
    } catch (error) { console.warn('[AI] Provider performance lookup failed; using configured preference:', error instanceof Error ? error.message : error); return [...configured].sort(provider => provider === preferred ? -1 : 1); }
  }

  private async syncCompletedHistory(daysBack: number): Promise<void> {
    try { const completed = await footballDataService.getCompletedMatches(daysBack); if (!completed.length) { console.log(`[History] No completed football-data.org matches found in the last ${daysBack} days.`); return; } await Promise.all(completed.map(match => this.storeFixture(match))); console.log(`[History] Saved ${completed.length} completed football-data.org matches for prediction history.`); }
