...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshSession = async () => { const token = getAuthToken(); if (!token) { setUser(null); setIsLoading(false); return; } try { const response = await fetch(`${API_BASE_URL}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('Session expired'); const data = await response.json(); setUser(data.user || null); } catch { localStorage.removeItem(TOKEN_KEY); setUser(null); } finally { setIsLoading(false); } };
  useEffect(() => { void refreshSession(); }, []);
  const signOut = async () => { const token = getAuthToken(); try { if (token) await fetch(`${API_BASE_URL}/api/auth/logout`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } }); } catch { /* local sign-out still succeeds */ } finally { localStorage.removeItem(TOKEN_KEY); setUser(null); } };
