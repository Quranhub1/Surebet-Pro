...
import { BrainCircuit, Loader2, RefreshCw, Target, Play } from 'lucide-react';
...
export function Dashboard() {
  ...
  const runAnalysisNow = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/football/analyze-now`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || `Analysis request returned ${response.status}`);
      setPredictions(Array.isArray(payload.predictions) ? payload.predictions : []);
      setUpdatedAt(payload.completedAt || new Date().toISOString());
    } catch (err) {
      console.error('Error running AI football analysis:', err);
      setError(err instanceof Error ? err.message : 'Manual football analysis failed.');
    } finally {
      ...
      <div className="w-full max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={runAnalysisNow} disabled={runningAnalysis || refreshing} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#39FF14] text-black px-4 py-2.5 text-sm font-bold hover:brightness-95 transition disabled:opacity-50">
              <Play className={`w-4 h-4 ${runningAnalysis ? 'animate-pulse' : ''}`} />
              {runningAnalysis ? 'Running analysis...' : 'Run analysis now'}
            </button>
            <button type="button" onClick={() => fetchPredictions(true)} disabled={refreshing || runningAnalysis} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#2c2e33] bg-[#161618] px-4 py-2.5 text-sm font-semibold hover:bg-[#1d1d20] transition-colors disabled:opacity-50">
          ...
          <div className="px-5 py-4 border-b border-[#2c2e33] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#8b8d93]">One shared AI analysis cycle is shown to every user until the next 12-hour cycle.</p>
              </div>
            </div>
            <div className="text-xs text-[#8b8d93]">{updatedAt ? `Updated ${new Date(updatedAt).toLocaleTimeString()}` : 'Waiting for analysis...'}</div>
          </div>

          {loading ? (
            <div className="p-10 flex items-center justify-center gap-3 text-sm text-[#8b8d93]"><Loader2 className="w-5 h-5 animate-spin" />AI is analyzing upcoming matches...</div>
          ) : error ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-[#8b8d93] mb-5">Run the analysis manually to fetch the latest fixtures and generate predictions.</p>
              <button type="button" onClick={runAnalysisNow} disabled={runningAnalysis} className="inline-flex items-center gap-2 rounded-xl bg-[#39FF14] text-black px-4 py-2.5 text-sm font-bold disabled:opacity-50"><Play className="w-4 h-4" />{runningAnalysis ? 'Running analysis...' : 'Run analysis now'}</button>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-[#2c2e33]">
              {predictions.map((prediction) => (
                <article key={prediction.id} className="bg-[#111113] p-6 hover:bg-[#171719] transition-colors">
                  ...
                  <div className="mt-4 pt-3 border-t border-[#2c2e33] flex items-center justify-between gap-3 text-[10px] text-[#6f727a]"><span>{formatProvider(prediction.aiProvider)}{prediction.aiModel ? ` · ${prediction.aiModel}` : ''}</span><span>AI analysis only</span></div>
                </article>
              ))}
            </div>
          )}
