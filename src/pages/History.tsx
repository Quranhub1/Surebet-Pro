...
const winnerName = (homeTeam: string, awayTeam: string, result: string | null) => result === 'Home' ? homeTeam : result === 'Away' ? awayTeam : result === 'Draw' ? 'Draw' : null;
const confidenceLabel = (value: number | null) => value == null ? '—' : value >= 75 ? 'High' : value >= 55 ? 'Medium' : 'Low';
const formatKickoff = (value: unknown) => { if (typeof value !== 'string' && typeof value !== 'number') return 'Date unavailable'; const date = new Date(value); if (Number.isNaN(date.getTime())) return 'Date unavailable'; return `${date.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })} · ${date.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}`; };

function normalizeHistoryRow(row: any): HistoryRow {
  ...
  const actualResultFromScore = winnerFromScore(actualHomeGoals, actualAwayGoals);
  const predictedWinner = row.predictedWinner || winnerName(row.homeTeam || 'Home', row.awayTeam || 'Away', predictedResult);
  const actualResult = row.actualResult || winnerName(row.homeTeam || 'Home', row.awayTeam || 'Away', actualResultFromScore);
  const derivedPredictionCorrect = row.predictionCorrect != null
    ? Boolean(row.predictionCorrect)
    : predictedResult != null && actualResultFromScore != null
      ? predictedResult === actualResultFromScore
      : null;
  const derivedScoreCorrect = row.scoreCorrect != null
  ...
  return {
    ...
    predictedWinner,
    actualResult,
    predictionCorrect: derivedPredictionCorrect,
    scoreCorrect: derivedScoreCorrect,
    ...
    kickoffAt: row.kickoffAt || row.kickoff || row.startTime || '',
  };
}
...
export function History() {
  ...
  const loadHistory = async () => {
    try {
      ...
      setRows(history.map(normalizeHistoryRow));
    } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    const providers = (['gemini', 'groq'] as const).map(name => { const list = evaluated.filter(row => row.aiProvider === name); return { provider: name, total: list.length, correct: list.filter(row => row.predictionCorrect).length, exact: list.filter(row => row.scoreCorrect).length, accuracy: list.length ? Math.round(list.filter(row => row.predictionCorrect).length / list.length * 100) : null }; });
    const leagues = Array.from(new Set(evaluated.map(row => row.league || 'Football'))).map(name => { const list = evaluated.filter(row => (row.league || 'Football') === name); const wins = list.filter(row => row.predictionCorrect).length; return { league: name, total: list.length, accuracy: Math.round(wins / list.length * 100) }; }).sort((a, b) => b.accuracy - a.accuracy || b.total - a.total).slice(0, 8);
  ...
  const filteredRows = useMemo(() => rows.filter(row => { const filterMatch = filter === 'all' || (filter === 'correct' && row.predictionCorrect === true) || (filter === 'missed' && row.predictionCorrect === false) || (filter === 'exact' && row.scoreCorrect === true); return filterMatch && (provider === 'all' || row.aiProvider === provider) && (league === 'all' || (row.league || 'Football') === league); }), [rows, filter, provider, league]);

  return <div className="mx-auto w-full max-w-7xl p-5 pb-16 md:p-8">
    <div className="mb-8 flex items-start justify-between gap-4"><div><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#39FF14]/10 text-[#39FF14]"><HistoryIcon className="h-6 w-6" /></div><h1 className="text-3xl font-extrabold text-white">Analysis History</h1></div><p className="mt-2 text-sm text-gray-400">Authoritative fixture dates, preserved predictions, and actual completed results used as evidence for future analysis.</p></div><button onClick={() => void loadHistory()} className="rounded-xl border border-[#333] bg-[#171717] px-4 py-2 text-sm font-bold text-gray-200 hover:bg-[#222]">Refresh</button></div>
    ...
    {loading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-9 w-9 animate-spin text-[#39FF14]" /></div> : error ? <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6 text-red-300">{error}</div> : filteredRows.length === 0 ? <div className="rounded-2xl border border-[#292929] bg-[#111] p-10 text-center"><CalendarClock className="mx-auto h-10 w-10 text-gray-600" /><h2 className="mt-4 text-lg font-bold text-white">No matching history</h2><p className="mt-2 text-sm text-gray-500">Completed predictions will appear here automatically, with the original prediction preserved.</p></div> : <div className="space-y-4">{filteredRows.map(row => { const actualWinner = winnerName(row.homeTeam, row.awayTeam, winnerFromScore(row.actualHomeGoals, row.actualAwayGoals)); const hasPrediction = row.predictedHomeGoals != null || row.predictedAwayGoals != null || Boolean(row.predictedWinner) || Boolean(row.analysis) || Boolean(row.advice); return <article key={row.id} className="overflow-hidden rounded-2xl border border-[#292929] bg-[#111]"><div className="flex flex-col gap-4 border-b border-[#242424] p-5 md:flex-row md:items-center md:justify-between"><div><div className="text-xs font-bold uppercase tracking-wider text-[#39FF14]">{row.league || 'Football'}</div><h2 className="mt-1 text-lg font-extrabold text-white">{row.homeTeam} <span className="text-gray-600">vs</span> {row.awayTeam}</h2><div className="mt-1 text-xs text-gray-500">{formatKickoff(row.kickoffAt)} · {row.status}</div></div><div className="flex flex-wrap items-center gap-2">{row.predictionCorrect === true ? <span className="flex items-center gap-1 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-400"><CheckCircle2 className="h-4 w-4" /> Winner correct</span> : row.predictionCorrect === false ? <span className="flex items-center gap-1 rounded-full bg-red-500/10 px-3 py-1 text-xs font-bold text-red-400"><XCircle className="h-4 w-4" /> Winner missed</span> : <span className="rounded-full bg-gray-500/10 px-3 py-1 text-xs font-bold text-gray-400">Awaiting evaluation</span>}{row.scoreCorrect === true && <span className="flex items-center gap-1 rounded-full bg-[#39FF14]/10 px-3 py-1 text-xs font-bold text-[#39FF14]"><Target className="h-4 w-4" /> Exact score</span>}</div></div><div className="grid gap-4 p-5 md:grid-cols-3"><div className="rounded-xl border border-[#292929] bg-[#151515] p-5"><div className="text-xs font-bold uppercase tracking-wider text-gray-500">Predicted</div><div className="mt-3 text-xl font-black text-white">{row.predictedHomeGoals ?? '—'} <span className="text-gray-600">-</span> {row.predictedAwayGoals ?? '—'}</div><div className="mt-1 text-sm text-gray-300">{row.predictedWinner || 'Not evaluated'}</div><div className="mt-3 flex items-center justify-between border-t border-[#292929] pt-3 text-xs text-gray-500"><span>Confidence: <b className="text-gray-300">{row.confidence == null ? '—' : `${row.confidence}%`}</b></span><span>{confidenceLabel(row.confidence)}</span></div></div><div className="rounded-xl border border-[#292929] bg-[#151515] p-5"><div className="text-xs font-bold uppercase tracking-wider text-gray-500">Actual</div><div className="mt-3 text-xl font-black text-white">{row.actualHomeGoals ?? '—'} <span className="text-gray-600">-</span> {row.actualAwayGoals ?? '—'}</div><div className="mt-1 text-sm text-gray-300">{row.actualResult || actualWinner || '—'}</div><div className="mt-3 border-t border-[#292929] pt-3 text-xs text-gray-500">{row.scoreCorrect ? 'Exact score matched' : row.predictionCorrect ? 'Winner correct, score differed' : row.predictionCorrect === false ? 'Predicted winner differed from actual result' : hasPrediction ? 'Prediction saved, evaluation unavailable' : 'No saved AI prediction for this fixture'}</div></div><div className="rounded-xl border border-[#292929] bg-[#151515] p-5"><div className="text-xs font-bold uppercase tracking-wider text-gray-500">AI analysis</div><p className="mt-3 max-h-28 overflow-y-auto text-sm leading-6 text-gray-300">{row.analysis || row.advice || 'No AI analysis saved for this fixture.'}</p><div className="mt-3 border-t border-[#292929] pt-3 text-xs text-gray-500">Provider: <span className="font-bold uppercase text-gray-300">{row.aiProvider || '—'}</span> · Model: {row.aiModel || 'not evaluated'}</div></div></div>{(row.keyFactors?.length > 0) && <div className="border-t border-[#242424] px-5 pb-5 pt-0"><div className="mb-2 text-xs font-bold uppercase tracking-wider text-gray-500">Key factors used</div><div className="flex flex-wrap gap-2">{row.keyFactors.map((factor, index) => <span key={`${row.id}-${index}`} className="rounded-lg bg-[#191919] px-3 py-1.5 text-xs text-gray-400">{factor}</span>)}</div></div>}</article>; })}</div>}
  </div>;
}
