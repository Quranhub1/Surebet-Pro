...
import { Activity, Settings, ShieldAlert, LogOut, X, PanelLeftClose, PanelLeftOpen, BrainCircuit, History as HistoryIcon } from 'lucide-react';
import { clsx } from 'clsx';
import { useAuth } from '../contexts/AuthContext';

const navItems = [
  { icon: BrainCircuit, label: 'AI Football Analysis', path: '/' },
  { icon: HistoryIcon, label: 'Analysis History', path: '/history' },
      ...
      <aside className={clsx('fixed md:relative inset-y-0 left-0 z-50 md:z-10 flex h-screen flex-col border-r border-[#222] bg-[#111111] transition-all duration-300 ease-in-out', collapsed ? 'md:w-20' : 'md:w-64', mobileOpen ? 'translate-x-0 w-72' : '-translate-x-full md:translate-x-0')}>
        <div className={clsx('flex items-center border-b border-[#222]', collapsed ? 'justify-center p-4' : 'justify-between p-5')}>
          <div className="flex items-center gap-3 min-w-0">
            ...
            {!collapsed && <span className="text-xl font-extrabold tracking-tight text-white">Football<span className="text-[#39FF14]">AI</span></span>}
          </div>
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto px-3 py-5">
          {visibleItems.map(item => <NavLink key={item.path} to={item.path} onClick={onCloseMobile} title={collapsed ? item.label : undefined} className={({ isActive }) => clsx('flex items-center rounded-xl py-2.5 text-sm font-semibold transition-all duration-200', collapsed ? 'justify-center px-2' : 'gap-3 px-4', isActive ? 'bg-[#39FF14]/10 text-[#39FF14]' : 'text-gray-400 hover:bg-[#1a1a1a] hover:text-white')}>
        ...
        <div className={clsx('space-y-3 border-t border-[#222] bg-[#0a0a0a]/50 p-3', collapsed && 'items-center')}>
          {!collapsed && <div className="rounded-xl border border-[#333] bg-[#1a1a1a] p-4"><div className="mb-1 truncate text-xs font-medium text-gray-500">{profile?.email}</div><div className="text-sm font-bold text-white">Football Analysis</div><div className="mt-1 text-xs text-gray-500">AI predictions and saved results</div></div>}
