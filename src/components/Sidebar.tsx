...
import { Activity, BarChart3, History, LogOut, X, PanelLeftClose, PanelLeftOpen, Brain, ShieldCheck } from 'lucide-react';
...
export function Sidebar({ collapsed, mobileOpen, onToggleCollapsed, onCloseMobile }: SidebarProps) {
  const { profile, signOut } = useAuth(); const [isAdmin, setIsAdmin] = useState(false);
  useEffect(() => { const token = getAuthToken(); if (!token) return; fetch(`${API_BASE_URL}/api/subscription`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' }).then(response => response.ok ? response.json() : null).then(data => setIsAdmin(Boolean(data?.isAdmin))).catch(() => setIsAdmin(false)); }, []);
  const items = isAdmin ? [...navItems, { icon: ShieldCheck, label: 'Admin Subscriptions', path: '/admin' }] : navItems;
  return <>
    ...
    <aside className={clsx('fixed md:relative inset-y-0 left-0 z-50 md:z-10 flex h-screen flex-col border-r border-[#222] bg-[#111111] transition-all duration-300 ease-in-out', collapsed ? 'md:w-20' : 'md:w-64', mobileOpen ? 'translate-x-0 w-72' : '-translate-x-full md:translate-x-0')}>
      <div className={clsx('relative flex items-center border-b border-[#222]', collapsed ? 'justify-center p-4' : 'justify-between p-5')}><div className="flex items-center gap-3 min-w-0"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#39FF14] shadow-[0_0_15px_rgba(57,255,20,0.3)]"><Activity className="h-5 w-5 text-black" strokeWidth={2.5} /></div>{!collapsed && <span className="text-xl font-extrabold tracking-tight text-white">Surebet<span className="text-[#39FF14]">Pro</span></span>}</div><button onClick={onCloseMobile} aria-label="Close navigation" title="Close navigation" className="absolute right-3 top-3 z-20 rounded-lg p-2 text-gray-300 hover:bg-[#1a1a1a] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#39FF14] md:hidden"><X className="h-5 w-5" /></button></div>
      <nav className="flex-1 space-y-1.5 overflow-y-auto px-3 py-5">{items.map(item => <NavLink key={item.path} to={item.path} onClick={onCloseMobile} title={collapsed ? item.label : undefined} className={({isActive}) => clsx('flex items-center rounded-xl py-2.5 text-sm font-semibold transition-all duration-200', collapsed ? 'justify-center px-2' : 'gap-3 px-4', isActive ? 'bg-[#39FF14]/10 text-[#39FF14]' : 'text-gray-400 hover:bg-[#1a1a1a] hover:text-white')}><item.icon className="h-5 w-5 shrink-0" />{!collapsed && <span className="truncate">{item.label}</span>}</NavLink>)}</nav>
